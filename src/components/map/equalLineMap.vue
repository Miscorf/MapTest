<template>
  <div style="height: 1000px; height: 800px">
    <div id="equal_line_map" style="width: 900px; height: 600px"></div>
  </div>
</template>

<script>
import { load_data_from_file } from "@/api/test";
export default {
  name: "equal_line_map",
  components: {},
  data() {
  ...
  mounted() {
    console.log("draw_equal_line_map");
    // option.series.data =
    this.draw_depth_map();
  },
  ...
  methods: {
    load_data() {
      load_data_from_file().then((response) => {
        console.log(response);
      });
    },
    draw_depth_map() {
      var chartDom = document.getElementById("equal_line_map");
      ...
      let line_data = generateLineDate();
